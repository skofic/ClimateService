...
###
prefix="twsan_m_euu_"
epoc="${path}/DroughtObservatory/TWS"
...
do
	...
	echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
	echo ">>> CONVERT WGS84 FROM TWS/download/${name} TO TWS/converted/${date}.tif"
	echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
	...
	echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
	echo ">>> CONVERT CSV FROM TWS/converted/${date}.tif TO TWS/CSV/${date}.csv"
	echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
