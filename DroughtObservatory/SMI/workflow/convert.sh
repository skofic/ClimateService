...
do
	...
	echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
	echo ">>> CONVERT WGS84 FROM SMI/download/${name} TO SMI/converted/${date}.tif"
	echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
	...
	echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
	echo ">>> CONVERT CSV FROM SMI/converted/${date}.tif TO SMI/CSV/${date}.csv"
	echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
...
elapsed=$((end-start))
echo "--------------------------------------------------"
echo "- CONVERT SMI FILES: $elapsed seconds"
echo "--------------------------------------------------"
echo ""
