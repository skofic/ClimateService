...
do
	...
	echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
	echo ">>> CONVERT WGS84 FROM FAPAR/download/${name} TO FAPAR/converted/${date}.tif"
	echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
	...
	echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
	echo ">>> CONVERT CSV FROM FAPAR/converted/${date}.tif TO FAPAR/CSV/${date}.csv"
	echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
...
elapsed=$((end-start))
echo "--------------------------------------------------"
echo "- CONVERT FAPAR FILES: $elapsed seconds"
echo "--------------------------------------------------"
echo ""
